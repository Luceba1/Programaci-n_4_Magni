...
export default function ParticipanteCard({ participante, onDelete, onEdit }: ParticipanteCardProps) {
  return (
    <article className={`rounded-lg border p-4 shadow transition hover:shadow-lg ${nivelStyles[participante.nivel]}`}>
      ...
      <p className={`mt-1 text-sm font-semibold ${textoNivelStyles[participante.nivel]}`}>Nivel: {participante.nivel}</p>
