...
export default function Formulario({ datos, error, editando, onChange, onSubmit }: FormularioProps) {
  const manejarTecnologias = (event: ChangeEvent<HTMLInputElement>) => {
    const { value, checked } = event.target;
    const nuevasTecnologias = checked
      ? [...datos.tecnologias, value]
      : datos.tecnologias.filter((t) => t !== value);
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <input type="text" value={datos.nombre} onChange={(e) => onChange('nombre', e.target.value)}
            placeholder="Nombre" className="w-full rounded border border-slate-300 px-3 py-2 outline-none transition focus:border-blue-500" />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium">Email</label>
          <input type="email" value={datos.email} onChange={(e) => onChange('email', e.target.value)}
            placeholder="Email" className="w-full rounded border border-slate-300 px-3 py-2 outline-none transition focus:border-blue-500" />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium">Edad</label>
          <input type="number" min="1" value={datos.edad} onChange={(e) => onChange('edad', e.target.value)}
            placeholder="Edad" className="w-full rounded border border-slate-300 px-3 py-2 outline-none transition focus:border-blue-500" />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium">País</label>
          <select value={datos.pais} onChange={(e) => onChange('pais', e.target.value)}
          ...
          <div className="flex flex-wrap gap-4">
            {modalidades.map((modalidad) => (
              <label key={modalidad} className="flex items-center gap-2 text-sm">
                <input type="radio" name="modalidad" value={modalidad}
                  checked={datos.modalidad === modalidad} onChange={(e) => onChange('modalidad', e.target.value)} />
                {modalidad}
              </label>
            ))}
          </div>
            ...
            {tecnologiasDisponibles.map((tecnologia) => (
              <label key={tecnologia} className="flex items-center gap-2 text-sm">
                <input type="checkbox" value={tecnologia}
                  checked={datos.tecnologias.includes(tecnologia)} onChange={manejarTecnologias} />
                {tecnologia}
              </label>
            ))}
          </div>
        ...
        <div className="md:col-span-2">
          <button type="submit"
            className={`rounded px-4 py-2 font-medium text-white transition ${editando ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-blue-600 hover:bg-blue-700'}`}>
            {editando ? 'Actualizar participante' : 'Registrar participante'}
          </button>
        </div>
      </form>
    </section>
