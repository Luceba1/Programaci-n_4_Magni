...
export default function Home() {
  const { participantes, agregar, editar, eliminar, resetear } = useParticipantes();
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 sm:px-6">
        <header className="rounded-lg bg-green-500 px-6 py-5 text-white shadow">
          ...
          <p className="mt-2 text-center text-sm sm:text-base">
            TP 5 · useReducer + Context + Editar
          </p>
        </header>
          ...
          {participantesFiltrados.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              {participantesFiltrados.map((p) => (
                <ParticipanteCard key={p.id} participante={p} onDelete={eliminar} onEdit={handleEditar} />
              ))}
            </div>
          )}
        </section>
